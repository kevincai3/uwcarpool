...
def main():
    c = CountPrint(1000)
    conn = engine.connect()
    results = conn.execute("SELECT posts.post_id, posts.message FROM posts WHERE post_id > 0 AND post_id < 50000 ORDER BY post_id ASC");
    ...
    for row in results:
        c.print(".")
        stage_1 = process(row.message, False, False)
        stage_2 = process(row.message, True, False)
        stage_3 = process(row.message)
