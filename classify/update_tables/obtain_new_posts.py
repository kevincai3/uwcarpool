...
def obtain_posts(start = 0, end = None, additional_sql = ''):
    query = f'SELECT p.post_id, p.message, p.posttime, dp.stage_3 FROM posts AS p LEFT JOIN derived_posts AS dp ON dp.post_id = p.post_id LEFT JOIN estimate_posts AS ep ON ep.post_id = p.post_id WHERE p.post_id >= {start}'
    if end != None:
