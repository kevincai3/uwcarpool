...
from detection.split_by_trips import split_by_trips
...
def update_estimate_posts_new(new_derived_posts):    

    posts_type = new_derived_posts.apply(
        lambda x: predict(dos_model, dos_vectorizer, x["stage_3"]), axis = 1)

    new_derived_posts["post_type"] = posts_type
    
    print('done post_type')

    new_derived_posts["trips"] = new_derived_posts.apply(
        lambda x: split_by_trips(x["stage_3"].split(',')), axis = 1)

    new_derived_posts = split_data_frame_list(new_derived_posts,"trips", output_type=list)    
 
    locs = new_derived_posts.apply(
        lambda x: route_detection_2(x["trips"].split(',')), axis = 1)
    ...
    print('done routes')
    dates = new_derived_posts.apply(
        lambda x: find_dates(x["trips"].split(','), x["posttime"]), axis = 1)
    best_dates = [x[0] for x in dates]
    all_dates = [x[1] for x in dates]

    times = new_derived_posts.apply(
        lambda x: find_times(x["trips"].split(','), x["posttime"]), axis = 1)
    best_times = [x[0] for x in times]
    all_times = [x[1] for x in times]
    ...
    new_estimate_posts = pd.DataFrame({
        ...
        "time":best_times,
        "all_times":all_times,
        "post_type":new_derived_posts["post_type"],
        "posttime": new_derived_posts["posttime"]})
    new_estimate_posts.astype({
        "date": "object",
        "time": "object",
